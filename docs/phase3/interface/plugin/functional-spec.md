...
- the floating popup host label now prefers `popupSnapshot.page.domain`; `edge.test` remains only as the demo fallback when the popup snapshot omits domain text
...
- real extension popup:
  ...
  - `Quick smoke test` submits a lightweight pre-execution scheduler request from current page context, now carrying shared page/runtime fields plus lightweight DOM snapshot context into the persisted scheduler request entry, and renders deterministic launch state/result
...
- the popup mirror active-run card now prefers real runtime empty-state context:
  - `runtime.nextAction`
  - then `runtime.auditState`
  - only then the fixed `No active run` copy
...
- In the popup mirror quick-action area, the `Pick element` helper copy now prefers the current popup locator (`Inspect locator ...`) and only falls back to the older hover/capture demo copy when locator context is absent.
- In the popup mirror current-page card, the status badge now prefers explicit popup `status`, then falls back to `ready` when page context already exists, and only then to the older fixed `recognized` label.
- In the popup mirror active-run card, the queue badge now prefers explicit `runtime.queueState`, then falls back to `runtime.auditState`, and only then to the older fixed `idle` label.
