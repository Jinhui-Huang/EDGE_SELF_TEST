...
- the footer `Queue pressure` line now also shows a lightweight source hint from additive `runStatus.queueStateSource`; when that marker is explicitly `NONE`, the screen stops using the older parent snapshot queue fallback and shows a run-local no-context copy instead, while older payloads that omit the marker entirely still keep the legacy snapshot fallback
...
- the footer `Last event` line now also shows a lightweight source hint from additive `runStatus.lastEventSource`; when that marker is explicitly `NONE`, the screen stops using the older parent snapshot timeline fallback and shows a run-local no-context copy instead, while older payloads that omit the marker entirely still keep the legacy snapshot fallback
