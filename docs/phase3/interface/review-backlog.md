...
- Remaining limits:
- `live-page` now prefers run-local live artifacts and returns an explicit `UNAVAILABLE` shell when they are absent; when artifacts are missing, that shell can still surface persisted scheduler request page/runtime/locator context instead of staying empty. `runtime-log` now prefers run-local `runtime.log` when present, otherwise falls back to scheduler-event-derived shaping, and when non-step scheduler runtime events are also unavailable it can emit a small persisted request-context shell so monitor still shows page/runtime/action/locator cues. `steps` now prefers run-local `report.json.steps[]` when present and preserves failed/skipped terminal semantics instead of collapsing them into `TODO`, but otherwise still falls back to scheduler-event-derived or placeholder shaping; `status` now prefers run-local `report.json` / `live-page.json` / artifact timestamps for stronger terminal/progress/current-page semantics and conservative fallback progress, and its scheduler fallback now prefers persisted request `pageUrl` / runtime context over the older thin `targetUrl` shell
