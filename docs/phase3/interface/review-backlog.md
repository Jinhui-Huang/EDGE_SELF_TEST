...
- Remaining limits:
- `live-page` now prefers run-local live artifacts and returns an explicit `UNAVAILABLE` shell when they are absent; when artifacts are missing, that shell can still surface persisted scheduler request page/runtime/locator context instead of staying empty. `runtime-log` now prefers run-local `runtime.log` when present, otherwise falls back to scheduler-event-derived shaping, and when non-step scheduler runtime events are also unavailable it can emit a small persisted request-context shell so monitor still shows page/runtime/action/locator cues. `steps` now prefers run-local `report.json.steps[]` when present and preserves failed/skipped terminal semantics instead of collapsing them into `TODO`; when those artifacts are absent it now falls back only to real scheduler `STEP_*` events and otherwise returns an empty list instead of a placeholder flow. `status` now prefers run-local `report.json` / `live-page.json` / artifact timestamps for stronger terminal/progress/current-page semantics and conservative fallback progress, and its scheduler fallback now prefers persisted request `pageUrl` / runtime context over the older thin `targetUrl` shell
...
- Remaining limits:
- `runtime-log` now prefers run-local `runtime.log` when present, then scheduler-event-derived shaping, and finally a persisted request-context shell when no usable runtime-log artifact or non-step scheduler event exists; `steps` now prefers run-local `report.json.steps[]` when present and preserves failed/skipped terminal semantics instead of collapsing them into `TODO`, but otherwise falls back only to real scheduler `STEP_*` events and returns an empty list when those are absent; `status` now prefers run-local `report.json` / `live-page.json` / artifact timestamps for stronger terminal/progress/current-page semantics and conservative fallback progress, but it still falls back to a scheduler-derived shell when those stronger artifacts are absent
...
- Remaining limits:
  ...
  - `Quick smoke test` now also carries richer popup/tab/content-script context through `SCHEDULER_REQUEST_CREATE`, but downstream execution/monitor/report consumers still do not actively surface most of those richer request fields yet
