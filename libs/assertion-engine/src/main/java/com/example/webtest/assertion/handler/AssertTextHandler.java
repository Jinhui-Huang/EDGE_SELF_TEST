...
public class AssertTextHandler implements AssertionHandler {
    ...
    public AssertTextHandler(ElementResolver elementResolver, PageController pageController) {
    ...
    public AssertionResult handle(StepDefinition step, ExecutionContext context) {
        ...
        if (!resolved.isFound()) {
            return AssertionResult.failure(step.getId(), "Expected text [" + step.getExpected() + "] but element was not found");
        }
        String actual = pageController.elementText(resolved.getBy(), resolved.getValue(), resolved.getIndex(), context);
        if (!Objects.equals(step.getExpected(), actual)) {
            return AssertionResult.failure(
                    step.getId(),
                    "Expected text [" + step.getExpected() + "] but was [" + actual + "]");
        }
        return AssertionResult.success(step.getId(), "text assertion success");
    }
}
